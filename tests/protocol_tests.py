...
class ProtocolBinaryCommandsTest(unittest.TestCase):
    ...
    def test_parsing_errors(self):
        ...
        self.assertRaises(
            ...
            bytearray(malformed_command_buffer % "DDDD", 'utf-8')
        )
        self.assertRaises(
            ProtocolError,
            protocol.parse_binary_command,
            bytearray(malformed_command_buffer % protocol.MAGIC_RES_STRING, 'utf-8'),
            is_response=False
        )
        self.assertRaises(
            ProtocolError,
            protocol.parse_binary_command,
            bytearray(malformed_command_buffer % protocol.MAGIC_REQ_STRING, 'utf-8'),
            is_response=True
        )

        # Raise unknown command errors
        unassigned_gearman_command = 1234
        unknown_command_buffer = struct.pack('!4sII', bytes(protocol.MAGIC_RES_STRING, 'utf-8'), unassigned_gearman_command, 0)
        #unknown_command_buffer = bytearray(unknown_command_buffer, 'utf-8')
        ...
        imaginary_command_buffer = struct.pack('!4sII4s', bytes(protocol.MAGIC_RES_STRING, 'utf-8'), protocol.GEARMAN_COMMAND_TEXT_COMMAND, 4, bytes('ABCD','utf-8'))
        #imaginary_command_buffer = bytearray(imaginary_command_buffer, 'utf-8')
        ...
        unexpected_payload_command_buffer = struct.pack('!4sII4s', bytes(protocol.MAGIC_RES_STRING, 'utf-8'), protocol.GEARMAN_COMMAND_NOOP, 4, bytes('ABCD','utf-8'))
        #unexpected_payload_command_buffer = bytearray(unexpected_payload_command_buffer, 'utf-8')
    ...
    def test_parsing_request(self):
        ...
        grab_job_command_buffer = struct.pack('!4sII', bytes(protocol.MAGIC_REQ_STRING, 'utf-8'), protocol.GEARMAN_COMMAND_GRAB_JOB_UNIQ, 0)
    ...
    def test_parsing_without_enough_data(self):
        ...
        not_enough_data_command_buffer = struct.pack('!4s', bytes(protocol.MAGIC_RES_STRING, 'utf-8'))
        ...
        not_enough_data_command_buffer = struct.pack('!4sII', bytes(protocol.MAGIC_RES_STRING, 'utf-8'), protocol.GEARMAN_COMMAND_ECHO_RES, 4)
        #not_enough_data_command_buffer = array.array("c", not_enough_data_command_buffer)
        cmd_type, cmd_args, cmd_len = protocol.parse_binary_command(not_enough_data_command_buffer)
        self.assertEqual(cmd_type, None)
        self.assertEqual(cmd_args, None)
    ...
    def test_parsing_no_args(self):
        noop_command_buffer = struct.pack('!4sII', bytes(protocol.MAGIC_RES_STRING, 'utf-8'), protocol.GEARMAN_COMMAND_NOOP, 0)
    ...
    def test_parsing_single_arg(self):
        echoed_string = 'abcd'
        echo_command_buffer = struct.pack('!4sII4s', bytes(protocol.MAGIC_RES_STRING, 'utf-8'), protocol.GEARMAN_COMMAND_ECHO_RES, 4, bytes(echoed_string, 'utf-8'))
    ...
    def test_parsing_single_arg_with_extra_data(self):
        ...
        excess_echo_command_buffer = struct.pack('!4sII9s', bytes(protocol.MAGIC_RES_STRING, 'utf-8'), protocol.GEARMAN_COMMAND_ECHO_RES, 4, bytes(excess_data, 'utf-8'))
    ...
    def test_parsing_multiple_args(self):
        ...
        uniq_command_buffer = struct.pack('!4sII%ds' % payload_size, bytes(protocol.MAGIC_RES_STRING, 'utf-8'), protocol.GEARMAN_COMMAND_JOB_ASSIGN_UNIQ, payload_size, bytes(binary_payload, 'utf-8'))
    ...
    def test_packing_response(self):
        ...
        expected_command_buffer = struct.pack('!4sII', bytes(protocol.MAGIC_RES_STRING, 'utf-8'), cmd_type, 0)
    ...
    def test_packing_no_arg(self):
        ...
        expected_command_buffer = struct.pack('!4sII', bytes(protocol.MAGIC_REQ_STRING, 'utf-8'), cmd_type, 0)
    ...
    def test_packing_single_arg(self):
        ...
        expected_command_buffer = struct.pack(expected_format, bytes(protocol.MAGIC_REQ_STRING, 'utf-8'), cmd_type, expected_payload_size, bytes(cmd_args['data'], 'utf-8'))
        packed_command_buffer = protocol.pack_binary_command(cmd_type, cmd_args)
        self.assertEqual(packed_command_buffer, expected_command_buffer)
    ...
    def test_packing_multiple_args(self):
        ...
        expected_format = '!4sII%ds' % expected_payload_size
        expected_command_buffer = struct.pack(expected_format, bytes(protocol.MAGIC_REQ_STRING, 'utf-8'), cmd_type, expected_payload_size, bytes(expected_payload, 'utf-8'))
...
class ProtocolTextCommandsTest(unittest.TestCase):
    ...
    def test_parsing_single_line(self):
        received_data = bytes("Hello there\n", 'utf-8')
        cmd_type, cmd_response, cmd_len = protocol.parse_text_command(received_data)
        self.assertEqual(cmd_type, protocol.GEARMAN_COMMAND_TEXT_COMMAND)
        self.assertEqual(cmd_response, dict(raw_text=received_data.decode().strip()))
        self.assertEqual(cmd_len, len(received_data))

    def test_parsing_multi_line(self):
        sentence_one = bytes("Hello there\n", 'utf-8')
        sentence_two = bytes("My name is bob\n", 'utf-8')
        received_data = sentence_one + sentence_two

        cmd_type, cmd_response, cmd_len = protocol.parse_text_command(received_data)
        self.assertEqual(cmd_type, protocol.GEARMAN_COMMAND_TEXT_COMMAND)
        self.assertEqual(cmd_response, dict(raw_text=sentence_one.decode().strip()))
