import logging
from python3_gearman.errors import UnknownCommandError
...
class GearmanCommandHandler(object):
    """A command handler manages the state which we should be in given a
    certain stream of commands
    ...
    def recv_command(self, cmd_type, **cmd_args):
        ...
        recv_command_function_name = gearman_command_name.lower(
        ).replace('gearman_command_', 'recv_')
