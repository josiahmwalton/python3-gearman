...
from python3_gearman import util

from python3_gearman.connection_manager import GearmanConnectionManager
from python3_gearman.admin_client_handler import \
    GearmanAdminClientCommandHandler
from python3_gearman.errors import ConnectionError, InvalidAdminClientState, \
    ServerUnavailable
from python3_gearman.protocol import GEARMAN_COMMAND_ECHO_RES, \
...
class GearmanAdminClient(GearmanConnectionManager):
    """GearmanAdminClient :: Interface to send/receive administrative commands
    to a Gearman server

    This client acts as a BLOCKING client and each call will poll until it
    receives a satisfactory server response
    ...
    def establish_admin_connection(self):
        ...
        self.current_handler = self.connection_to_handler_map[
    ...
    def ping_server(self):
        ...
        server_response = self.wait_until_server_responds(
    ...
    def get_workers(self):
        """Retrieves a list of workers and reports what tasks they're
        operating on"""
        self.establish_admin_connection()
    ...
    def wait_until_server_responds(self, expected_type):
        ...
        if not self.current_handler.response_ready:
            raise InvalidAdminClientState(
                'Admin client timed out after %f second(s)' %
                self.poll_timeout)

        cmd_type, cmd_resp = self.current_handler.pop_response()
        if cmd_type != expected_type:
            raise InvalidAdminClientState(
                'Received an unexpected response... got command ' +
                '%r, expecting command %r' % (cmd_type, expected_type)
            )
