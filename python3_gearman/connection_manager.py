...
from python3_gearman.constants import _DEBUG_MODE_
from python3_gearman.errors import ConnectionError, ServerUnavailable
from python3_gearman.job import GearmanJob, GearmanJobRequest
...
class GearmanConnectionManager(object):
    ...
    def add_connection(self, hostport_tuple):
        ...
        gearman_host, gearman_port = (
            python3_gearman.util.disambiguate_server_parameter(hostport_tuple)
        )

        client_connection = self.connection_class(
            host=gearman_host, port=gearman_port)
    ...
    def establish_connection(self, current_connection):
        ...
        # Handler to connection map for
        # CommandHandler -> Connection interactions
        # Connection to handler map for Connection -> CommandHandler
        # interactions
    ...
    def poll_connections_once(self, submitted_connections, timeout=None):
        ...
        select_connections = set(
            current_connection for current_connection in submitted_connections
            if current_connection.connected
        )
        ...
        while not successful_select and select_connections:
            ...
            check_rd_connections = [
                current_connection for current_connection in select_connections
                if current_connection.readable()
            ]
            check_wr_connections = [
                current_connection for current_connection in select_connections
                if current_connection.writable()
            ]

            try:
                rd_list, wr_list, ex_list = python3_gearman.util.select(
                    check_rd_connections,
                    check_wr_connections,
                    select_connections,
                    timeout=timeout
                ...
                # On any exception, we're going to assume we ran into a socket
                # exception
                ...
                for conn_to_test in select_connections:
                    try:
                        _, _, _ = python3_gearman.util.select(
                            [conn_to_test], [], [], timeout=0)
                    except (select_lib.error, ConnectionError):
        ...
        if _DEBUG_MODE_:
            gearman_logger.debug(
                'select :: Poll - %d :: Read - %d :: Write - %d :: Error - %d',
                len(select_connections),
                len(rd_connections),
                len(wr_connections),
                len(ex_connections))
    ...
    def handle_connection_activity(
            ...
            wr_connections,
            ex_connections):
        """Process all connection activity... executes all
        handle_* callbacks"""
    ...
    def poll_connections_until_stopped(
            ...
            callback_fxn,
            timeout=None):
        """Continue to poll our connections until we receive a
        stopping condition"""
        stopwatch = python3_gearman.util.Stopwatch(timeout)

        any_activity = False
        callback_ok = callback_fxn(any_activity)
        connection_ok = compat.any(
            current_connection.connected for current_connection in
            submitted_connections
        )
        ...
        if not connection_ok:
            raise ServerUnavailable(
                'Found no valid connections in list: %r' %
                self.connection_list)
    ...
    def handle_error(self, current_connection):
        dead_handler = self.connection_to_handler_map.pop(
    ...
    def read_command(self, command_handler):
        ...
        NOTE:
            CommandHandlers have NO knowledge as to which connection they're
            representing
    ...
    def on_gearman_error(self, error_code, error_text):
        gearman_logger.error(
            'Received error from server: %s: %s' %
