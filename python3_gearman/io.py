...
def get_connection_poller():
    ...
    either be:
        ...
        - On all other systems: python3_gearman.io._Select: an object that
          mimics select.epoll, but uses select.select
    """
...
def _find_bad_connections(connections):
    ...
    for conn in connections:
        try:
            _, _, _ = python3_gearman.util.select([conn], [], [], timeout=0)
        except (select.error, python3_gearman.errors.ConnectionError):
            bad.append(conn)
    return bad
...
class _Select(object):
    ...
    def register(self, fd, evmask):
        ...
        Events are similar to those accepted by select.epoll:
            - python3_gearman.io.READ: report when fd is readable (i.e.: a
              socket.recv operation likely won't block, and will yield some
              data)
            - python3_gearman.io.WRITE: report when fd is writable (i.e.: a
              socket.send operation likely won't block, and will be able to
              write some data)
            - python3_gearman.io.ERROR: report when fd is in an error state
        """
    ...
    def poll(self, timeout):
        ...
        while not success and connections:
            connections -= errors
            try:
                r, w, e = python3_gearman.util.select(
